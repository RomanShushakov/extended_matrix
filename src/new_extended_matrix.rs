...
use crate::functions::
{
    new_matrices_dimensions_conformity_check, new_copy_element_value, remove_zero_values,
        ...
        -> Result<Self, &'a str>
    {
        ...
        while row < shape.0
        {
            let mut column = T::from(0u8);
            {
                while column < shape.1
                {
                    let current_lhs_element_value = new_copy_element_value(row, column,
                        self.basic_matrix.ref_matrix_type(),
                        self.basic_matrix.ref_elements_values());
                    let current_rhs_element_value = new_copy_element_value(row, column,
                            other.basic_matrix.ref_matrix_type(),
                            other.basic_matrix.ref_elements_values());
                    let element_value =
                        {
                            match operation
                            {
                    ...
                    if element_value.into().abs() > self.tolerance.into()
                    {
                        ...
                        basic_matrix.add_element(matrix_element_position, element_value);
                    }
