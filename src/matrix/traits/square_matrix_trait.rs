...
fn pivot_lup<V, SMT, VT>(a: &mut SMT, o: &mut [usize], s: &[<SMT as BasicOperationsTrait>::Value], n: usize, k: usize)
where V: FloatTrait,
      SMT: SquareMatrixTrait,
      VT: VectorTrait,
      <SMT as BasicOperationsTrait>::Value: FloatTrait<Output = V>,
...
{
    let mut p = k;
    let mut big = (*a.get_element_value(&Position(o[k], k))
        .expect("Element is absent") / s[o[k]]).my_abs();
    for ii in k + 1..n
    {
        let dummy = (*a.get_element_value(&Position(o[ii], k))
            .expect("Element is absent") / s[o[ii]]).my_abs();
        if dummy > big
        {
            big = dummy;
            p = ii;
...
fn decompose_lup<V, SMT, VT>(a: &mut SMT, n: usize, rel_tol: <SMT as BasicOperationsTrait>::Value,
    o: &mut [usize], s: &mut [<SMT as BasicOperationsTrait>::Value]) -> Result<(), String>
    where V: FloatTrait,
          SMT: SquareMatrixTrait + BasicOperationsTrait<Value = V>,
          VT: VectorTrait + BasicOperationsTrait<Value = V>,
          <SMT as BasicOperationsTrait>::Value: FloatTrait<Output = V>,
          <VT as BasicOperationsTrait>::Value: FloatTrait<Output = V>,
{
    ...
    for k in 0..n - 1
    {
        ...
        if (*a.get_element_value(&Position(o[k], k)).expect("Element is absent") / s[o[k]]).my_abs() < rel_tol
        {
            return Err("Ill conditioned system".to_string());
        }
        for i in k + 1..n
        {
            let factor = *a.get_element_value(&Position(o[i], k)).expect("Element is absent") /
                *a.get_element_value(&Position(o[k], k)).expect("Element is absent");
            *a.get_mut_element_value(&Position(o[i], k)).expect("Element is absent") = factor;
            for j in k + 1..n
            {
                *a.get_mut_element_value(&Position(o[i], j)).expect("Element is absent") = 
                    *a.get_element_value(&Position(o[i], j)).expect("Element is absent") - factor * 
                    *a.get_element_value(&Position(o[k], j)).expect("Element is absent");
            }
        }
    }
    if (*a.get_element_value(&Position(o[n - 1], n - 1)).expect("Element is absent") / s[o[n - 1]])
        .my_abs() < rel_tol
    {
        return Err("Ill conditioned system".to_string());
    }
          ...
          VT2: VectorTrait + BasicOperationsTrait<Value = V>,
{
    for i in 1..n
    {
        let mut sum = *b.get_element_value(&Position(o[i], 0)).expect("Element is absent");
        for j in 0..i
        {
            sum -= *a.get_element_value(&Position(o[i], j)).expect("Element is absent") * 
                *b.get_element_value(&Position(o[j], 0)).expect("Element is absent");
        }
        *b.get_mut_element_value(&Position(o[i], 0)).expect("Element is absent") = sum;
    }
    *x.get_mut_element_value(&Position(n - 1, 0)).expect("Element is absent") = 
        *b.get_element_value(&Position(o[n - 1], 0)).expect("Element is absent") / 
        *a.get_element_value(&Position(o[n - 1], n - 1)).expect("Element is absent");
    for i in (0..n - 1).rev()
    {
        let mut sum = V::from(0f32);
        for j in i + 1..n
        ...
        *x.get_mut_element_value(&Position(i, 0)).expect("Element is absent") = 
            (*b.get_element_value(&Position(o[i], 0)).expect("Element is absent") - sum) / 
            *a.get_element_value(&Position(o[i], i)).expect("Element is absent");
    }
}
