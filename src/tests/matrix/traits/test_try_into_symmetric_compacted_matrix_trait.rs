...
{
    let sm_1 = SquareMatrix::create(4, &[
            5.0, -4.0, 1.0, 0.0,
            -4.000001, 6.0, -4.0, 1.0,
            1.0, -4.0, 6.0, -4.0,
            0.0, 1.0, -4.0, 5.0,
        ]);
    let sm_2 = SquareMatrix::create(5, &[
        2.0, -2.0, 0.0, 0.0, -1.0,
        -2.0, 3.0, -2.0, 0.0, 0.0,
        0.0, -2.0, 5.0, -3.0, 0.0,
    ...
    let sm_3 = SquareMatrix::create(4, &[
        5.0, -4.0, 1.0, 0.0,
        -4.000001, 6.0, -4.0, 1.0,
        1.0, -4.0, 6.0, -4.0,
        0.0, 1.0, -4.0, 0.0,
    ]);
    let sm_4 = SquareMatrix::create(4, &[
        5.0, -4.0, 1.0, 0.0,
        -4.25, 6.0, -4.0, 1.0,
        1.0, -4.0, 6.0, -4.0,
        0.0, 1.0, -4.0, 0.0,
    ...
    assert_eq!(sm_1.try_into_symmetric_compacted_matrix(1e-6), Ok((expected_a_1, expected_maxa_1)));
    assert_eq!(sm_2.try_into_symmetric_compacted_matrix(1e-6), Ok((expected_a_2, expected_maxa_2)));
    assert_eq!(sm_3.try_into_symmetric_compacted_matrix(1e-6), 
        Err("Diagonal element [3, 3] equals to zero!".to_string()));
    assert_eq!(sm_4.try_into_symmetric_compacted_matrix(1e-6), 
...
fn test_forced_into_symmetric_compacted_matrix()
{
    ...
    assert_eq!(sm_1.forced_into_symmetric_compacted_matrix(1e-6, &mut warnings_1), 
        (expected_a_1, expected_maxa_1));
    assert_eq!(warnings_1, expected_warnings_1);
    assert_eq!(sm_2.forced_into_symmetric_compacted_matrix(1e-6, &mut warnings_2), 
        (expected_a_2, expected_maxa_2));
    assert_eq!(warnings_2, expected_warnings_2);
    assert_eq!(sm_3.forced_into_symmetric_compacted_matrix(1e-6, &mut warnings_3), 
